...
class RegisterView(APIView):
    """Public: Submit adoption registration request (no credentials yet)."""
    parser_classes = (MultiPartParser, FormParser)

    def post(self, request):
        serializer = RegisterSerializer(data=request.data)
        if serializer.is_valid():
            instance = serializer.save(is_approved=False, user_id="", password="")
            ...
            # ✅ Only confirmation email (NO credentials)
            subject = 'HopeNest Registration Received'
            from_email = settings.EMAIL_HOST_USER
            text_content = f"Dear {greeting_names},\n\nThank you for registering with HopeNest. " \
                           "Your application is under review by our admin. " \
                           "You will be notified once it is approved or declined."
            html_content = f"<p>Dear <strong>{greeting_names}</strong>,</p>" \
                           "<p>Thank you for registering with HopeNest. " \
                           "Your application is under review by our admin. " \
                           "You will be notified once it is approved or declined.</p>"
